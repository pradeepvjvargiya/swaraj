...
@section('content')
                ...
                <div class="container">
                    <div class="row">
                        @foreach ($teams as $key => $team)
                            <div class="col-lg-4 col-md-6 col-sm-6 m-b30 wow fadeInUp" data-wow-duration="2s"
                                data-wow-delay="0.3s">
                                <div class="dlab-box service-box-2">
                                    <div class="dlab-media radius-sm dlab-img-overlay1 dlab-img-effect rotate">
                                        <img src="{{ asset('images/team/' . $team->picture) }}" alt="{{ $team->name }}"
                                            style="max-height:300px">
                                    </div>
                                    <div class="dlab-info">
                                        <h4 class="title">{{ $team->name }}</h4>
                                        <h6>{{ $team->designation }}</h6>
                                        <p style="text-align:justify">{{ Str::limit($team->description, 200) }}...</p>
                                        <p style="display:none" id="bio_{{ $key }}">{{ $team->description }}</p>
                                        <a href="javascript:void()"
                                            onclick="openModal('{{ $key }}', '{{ $team->name }}')"
                                            class="site-button btnhover14">Read More</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
    ...
    <!--END Content-->
    <div class="modal fade" id="bioModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
